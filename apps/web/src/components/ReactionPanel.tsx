...
const AUDIENCE_TITLES: Record<ReactionSignal['audience'], string> = {
...
/**
 * One audience's reaction. The level decides the colour, the band decides the words, and the
 * reading that selected the band is printed underneath so the judgement can be checked.
 */
    ...
    <section className={`reaction reaction--${audience}`} aria-labelledby={`reaction-${audience}`}>
      ...
      {mine.map((signal) => (
        <div key={signal.id} className={`signal signal--${signal.level}`}>
          <p className="signal__headline">{signal.headline}</p>
